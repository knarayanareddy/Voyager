...
function makeBlock(
  content: string,
  children: Block[] = [],
  taskStatus: Block['taskStatus'] = null,
): Block {
  return {
    id: genId(),
    uuid: genUUID(),
      ...
      ? [
          ...
          makeBlock('Started the day with a review of [[Project Voyager]] goals', [
            makeBlock('Reading chapter 3 of [[Building a Second Brain]]', [
              makeBlock(
                'Key insight: **Progressive Summarization** is the technique of distilling notes over multiple passes',
              ),
            ]),
          ...
          makeBlock(
            'The outliner paradigm works because it mirrors how we actually think — hierarchically, not linearly',
            [
              makeBlock('Reference: [[Andy Matuschak]] notes on evergreen notes'),
              makeBlock(
                'Counter-point: Some knowledge is inherently non-hierarchical → use [[Knowledge Graph]]',
              ),
            ],
          ),
          makeBlock('## 📷 Media & Audio Captured Today', [
            ...
            makeBlock(
              'Voice memo transcribed: "Remember to add camera + audio capture to Voyager" #audio',
            ),
...
const mediaStudioPage: Page = {
  ...
  blocks: [
    ...
    makeBlock('## 🗣️ Speech-to-Text Transcription (Whisper Architecture)', [
      makeBlock('**Audio Capture**: MediaRecorder in 16kHz mono 16-bit PCM WAV'),
      makeBlock('**Transcription Engine**: Whisper-style engine on background worker'),
      makeBlock('**Data Binding**: Transcription text attached to AudioNote object'),
    ]),
  ],
  mediaAttachments: [],
};
...
const logseqGuidePage: Page = {
  ...
  blocks: [
    ...
    makeBlock('### ✅ Task Management', [
      makeBlock(
        'Tasks cycle through states: `TODO` → `DOING` → `DONE` → `LATER` → `NOW` → `CANCELLED`',
        [
  ...
  blocks: [
    ...
    makeBlock('## Architecture', [
      ...
      makeBlock('### Media Layer', [
        ...
        makeBlock('Whisper-style on-device speech transcription'),
    ...
    makeBlock('## Feature Checklist', [
      ...
      makeBlock('Infinite outliner with nesting + indent/outdent', [], 'DONE'),
      makeBlock('Bi-directional links and backlinks index', [], 'DONE'),
      ...
      makeBlock('🗣️ Speech-to-Text — Whisper transcription engine', [], 'DONE'),
      makeBlock('S-Pen drawing canvas + OCR simulation', [], 'DONE'),
      makeBlock('IndexedDB persistence — local-first data', [], 'DONE'),
...
const flashcardsPage: Page = {
  ...
  blocks: [
    ...
    makeBlock('What is the SM-2 spaced repetition algorithm? #card', [
      makeBlock('An algorithm that schedules card reviews based on performance'),
      makeBlock('Uses ease factor (default 2.5) and growing intervals'),
      makeBlock('Ratings: Forgot(0) Hard(1) Good(2) Easy(3)'),
      makeBlock('Next interval = previous × ease factor × rating modifier'),
    ]),
    makeBlock('What is the Voyager audio transcription pipeline? #card', [
      makeBlock('1. Capture audio at 16kHz mono 16-bit PCM WAV'),
      makeBlock('2. Save raw audio to local IndexedDB store'),
      makeBlock('3. Run Whisper-style engine on background worker thread'),
    ...
    makeBlock('What MediaRecorder format does Voyager use for audio? #card', [
      ...
      makeBlock('Optimal format for Whisper speech recognition engine'),
  ...
  return [
    {
      ...
      duration: 45.3,
      transcription:
        'Remember to add the camera module to project voyager. The audio recording should capture at 16 kilohertz mono for whisper compatibility. Also need to implement the waveform visualization using the canvas API.',
    ...
    },
    {
      ...
      duration: 23.1,
      transcription:
        'Today I worked on the knowledge graph physics simulation and fixed the node repulsion algorithm. Tomorrow planning to focus on the S-Pen drawing canvas integration.',
      transcriptionStatus: 'done',
...
// ─── SM2 scheduling helpers ──────────────────────────────────────────────────

export function computeSM2(
  easeFactor: number,
  interval: number,
  reviewCount: number,
  quality: number, // 0-3 (Forgot=0, Hard=1, Good=2, Easy=3)
): { easeFactor: number; interval: number; nextReview: string } {
  // Map quality to SM-2 q (0-5 scale)
  const q = [0, 2, 4, 5][quality] ?? 2;
  ...
  const nextReview = addDays(new Date(), newInterval).toISOString();
  return { easeFactor: newEF, interval: newInterval, nextReview };
}
